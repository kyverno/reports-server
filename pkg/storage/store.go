...
func New(embedded bool, config *db.PostgresConfig, clusterId string, etcdCfg *etcd.EtcdConfig) (Interface, error) {
	klog.Infof("setting up storage, debug=%v", embedded)
	...
	if embedded {
		...
		storage, err = db.New(config, clusterId)
		if err != nil {
			return nil, err
		}
	}
